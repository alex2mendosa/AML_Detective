...
final_summary_prompt = """
  ...
  You will receive two evidence blocks:

  1. OpenSanctions screening result — structured JSON from Stage-1 screening, AUTHORITATIVE for direct sanctions/PEP/regulatory listings.
  2. Public-source evidence — adverse media claims gathered from web sources.

  Both blocks must be considered. The OpenSanctions block always takes precedence for sanctions classification.


  ═══════════════════════════════════════════════════════════════
  SANCTIONS SCREENING OVERRIDE RULES — APPLY FIRST
  ═══════════════════════════════════════════════════════════════

  Read the `risk_level` field of the OpenSanctions block. Apply these rules before anything else:

  • risk_level = "RED"
      - scor_risc ≥ 76 (CRITICAL band)
      - recomandare_relatie_afaceri = "TERMINATE"
      - In analiza_suspiciuni, quote entity_id, sanctions_programs (program titles), risks (topic descriptions), and last_seen.
      - If sanctioned_persons is non-empty, list each name + role.

  • risk_level = "YELLOW"
      - scor_risc ≥ 51 (HIGH band)
      - recomandare_relatie_afaceri = "ENHANCED_DUE_DILIGENCE"
      - In analiza_suspiciuni, cite the specific topics and sanctions_programs that triggered the YELLOW verdict.

  • risk_level = "UNDEFINED" or "ERROR"
      - scor_risc cannot be reliably calculated
      - recomandare_relatie_afaceri = "SUSPEND"
      - In rezumat_analiza, state that sanctions screening could not be completed and manual verification is required before any partnership decision.

  • risk_level = "GREEN"
      - No direct sanctions hit. Proceed with normal analysis of the public-source evidence block.

  Always cite the OpenSanctions reason and aml_conclusion / sanction_conclusion fields verbatim in concluzie_finala when risk_level is RED or YELLOW.
  ...
  1. rezumat_analiza (Executive Summary)
     ...
     • If sanctions screening returned RED/YELLOW/UNDEFINED, state this explicitly in the first paragraph.

  2. scor_risc (Risk Score 0-100)
     Calculate using this scale (subject to the sanctions override above):

     CRITICAL (76-100):
     - Active sanctions listing (OpenSanctions risk_level=RED)
     ...
     HIGH (51-75):
     - OpenSanctions risk_level=YELLOW (linked/debarment/regulatory warning)
     ...
     LOW (0-25):
     - No sanctions hit AND old violations (pre-2020) fully resolved
     - No ongoing investigations
     - Strong compliance improvements
     - Clean recent record

  3. analiza_suspiciuni (Violation Analysis)
     For EACH item (sanctions hit OR media-derived violation), document:
     ...
     • Quote exact evidence with source (URL or OpenSanctions dataset name)

  4. situatie_actuala (Current Status)
     Document as of {current_date_alias}:

     ACTIVE SANCTIONS LISTINGS (from OpenSanctions block):
     • Program name(s) and dataset(s)
     • last_seen date
     • Any frozen-asset or wanted topics
  ...
  6. recomandare_relatie_afaceri (Business Recommendation)
     ...
     • SUSPEND - Pause until investigations resolve or screening completes
     ...
     Then provide:
     • Rationale tied to risk score AND sanctions verdict
  ...
  7. concluzie_finala (Final Conclusion)
     ...
     • What are the 3 most critical risks (sanctions + media combined)?
  ...
  ✓ The OpenSanctions block overrides the media block for sanctions classification


  ═══════════════════════════════════════════════════════════════
  EMPTY EVIDENCE OVERRIDE — APPLY ONLY IF BOTH CONDITIONS HOLD
  ═══════════════════════════════════════════════════════════════

  Apply this override ONLY when BOTH of these are true:

    (a) The OpenSanctions block has risk_level = "GREEN", or the block contains
        "No sanctions screening data available for this entity."
    (b) The public-source evidence block contains "No evidence available"
        or has no concrete claims with dates/amounts/authorities.

  If BOTH conditions hold, ignore the field-by-field instructions and return:

    scor_risc: 0
    rezumat_analiza: "Nu au fost identificate informații de presă negativă
                      pentru {entity_name_alias} în sursele consultate,
                      iar screening-ul OpenSanctions nu a returnat nicio coincidență."
    analiza_suspiciuni: "Nu au fost identificate suspiciuni."
    situatie_actuala: "Nu au fost identificate investigații, sancțiuni active sau
                       acțiuni de reglementare la data de {current_date_alias}."
    ...
    recomandare_relatie_afaceri: "CONTINUE — nu există motive de îngrijorare
                                  identificate în sursele publice sau în
  ...
  If (a) is false (i.e. sanctions risk_level is RED / YELLOW / UNDEFINED / ERROR),
  this override DOES NOT apply — proceed with the field-by-field instructions and
  the sanctions override rules above.

  Do NOT speculate. Absence of evidence is not evidence of risk — but presence of
  a sanctions hit IS evidence of risk regardless of media coverage.
